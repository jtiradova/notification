import type { PropDef, GetPropDefTypes } from "@singlestore/fusion/props";
import { asChildPropDef, gapPropDefs } from "@singlestore/fusion/props";
...
function parseGridValue(value: string): string {
    if (
        (gridPropDefs.columns.values as ReadonlyArray<string>).includes(value)
    ) {
        return value;
    }
